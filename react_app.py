"""
AI Code Review Assistant - React Backend
Serves React frontend and provides API endpoints for code analysis
"""
...
review_evaluator = ReviewEvaluator(None, gemini_client)
...
def analyze_code():
    ...
    try:
        ...
        # Create a simplified analysis using direct Gemini client
        analysis_result = analyze_code_directly(code, context, gemini_client)
...
if __name__ == '__main__':
    ...
    if not build_path.exists():
        logger.warning("React build not found. Please run 'npm run build' in the frontend directory")
        print("\nTo build the React frontend:")
        print("1. cd frontend")
        print("2. npm run build")
        print("3. cd ..")
        print("4. python react_app.py")
        sys.exit(1)
    
    port = int(os.environ.get('PORT', 3001))
    host = os.environ.get('HOST', '127.0.0.1')
    
    logger.info(f"Starting AI Code Review Assistant on http://{host}:{port}")
    print(f"\n🚀 AI Code Review Assistant starting on http://{host}:{port}")
    print("📝 Paste your git diff or code for comprehensive AI analysis")
    print("💡 Features: 16 quality criteria, syntax highlighting, export results")
